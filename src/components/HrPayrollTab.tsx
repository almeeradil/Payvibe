    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
        <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-xs">
          ...
          <div className="text-[10px] text-slate-500 mt-0.5">Biometric &amp; App Enrolled</div>
        </div>
        ...
        <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-xs">
          ...
          <div className="text-[10px] text-slate-500 mt-0.5">Across all store branches</div>
        </div>
        ...
        <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-xs flex items-center justify-between gap-2">
          <button
      ...
      <div className="flex border-b border-slate-200 space-x-4 text-xs font-bold">
        <button
          ...
          className={`pb-2.5 flex items-center gap-1.5 transition ${
            activeSubTab === 'payroll' ? 'border-b-2 border-indigo-600 text-indigo-600' : 'text-slate-500 hover:text-slate-800'
          }`}
        >
          <DollarSign className="w-4 h-4" />
        ...
        <button
          ...
          className={`pb-2.5 flex items-center gap-1.5 transition ${
            activeSubTab === 'attendance' ? 'border-b-2 border-indigo-600 text-indigo-600' : 'text-slate-500 hover:text-slate-800'
          }`}
        >
        ...
        <button
          ...
          className={`pb-2.5 flex items-center gap-1.5 transition ${
            activeSubTab === 'staff' ? 'border-b-2 border-indigo-600 text-indigo-600' : 'text-slate-500 hover:text-slate-800'
          }`}
        >
                ...
                {payrolls.map(p => {
                  ...
                  return (
                    <tr key={p.id} className="hover:bg-slate-50">
                      <td className="p-3 font-semibold text-slate-600">{p.month} {p.year}</td>
                      <td className="p-3 font-bold text-slate-900">{p.employeeName}</td>
                      <td className="p-3 text-slate-600">{p.totalDaysPresent} Days ({p.totalHoursWorked} hrs)</td>
                      ...
                      <td className="p-3 text-right">
                        <button
                          ...
                          className="px-2.5 py-1 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded font-bold text-[10px] inline-flex items-center gap-1 transition"
                        >
            ...
            <tbody className="divide-y divide-slate-100">
              {attendance.map(a => (
                <tr key={a.id} className="hover:bg-slate-50">
                  ...
                  <td className="p-3 font-mono text-slate-700">{a.punchInTime}</td>
                  <td className="p-3 font-mono text-slate-700">{a.punchOutTime || '-'}</td>
                  <td className="p-3 text-right font-black text-slate-900">{a.hoursWorked} hrs</td>
                  <td className="p-3 text-right font-bold text-cyan-600">+{a.overtimeHours} hrs</td>
                  <td className="p-3 text-slate-600 flex items-center gap-1.5 mt-1">
      ...
      {activeSubTab === 'staff' && (
        <div className="space-y-4">
          <div className="flex justify-between items-center bg-white p-4 rounded-xl border border-slate-200 shadow-xs">
            <h4 className="text-xs font-bold text-slate-800 uppercase">Staff &amp; Human Resource Profiles</h4>
            <button
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {employees.map(emp => (
              <div key={emp.id} className="bg-white p-5 rounded-xl border border-slate-200 shadow-xs hover:border-indigo-300 transition">
                ...
                <div className="mt-3 pt-3 border-t border-slate-100 text-xs space-y-1">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="text-slate-600 font-semibold">{emp.phone}</span>
                  </div>
                </div>
              </div>
            ))}
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-2xl max-w-md w-full p-6 border border-slate-200">
            <div className="flex justify-between items-center pb-3 border-b border-slate-200 mb-4">
              ...
              <button onClick={() => setShowPunchModal(false)} className="text-slate-400 hover:text-slate-600">✕</button>
            </div>
            <form onSubmit={handleSavePunch} className="space-y-3 text-xs">
              <div>
                <label className="block text-[10px] font-bold uppercase text-slate-500 mb-1">Select Employee *</label>
                <select
                  value={punchEmpId}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[10px] font-bold uppercase text-slate-500 mb-1">Punch Mode</label>
                  <select
                    value={punchMethod}
                ...
                <div>
                  <label className="block text-[10px] font-bold uppercase text-slate-500 mb-1">Attendance Status</label>
                  <select
                    value={punchStatus}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[10px] font-bold uppercase text-slate-500 mb-1">Hours Worked</label>
                  <input
                    type="number"
                    step="0.25"
                    value={punchHours}
                ...
                <div>
                  <label className="block text-[10px] font-bold uppercase text-slate-500 mb-1">Overtime Hours</label>
                  <input
                    type="number"
                    step="0.25"
                    value={punchOvertime}
      ...
      {showGeneratePayrollModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-2xl max-w-md w-full p-6 border border-slate-200">
            <div className="flex justify-between items-center pb-3 border-b border-slate-200 mb-4">
              <h3 className="font-extrabold text-slate-900 text-sm">Calculate &amp; Disburse Salary</h3>
              <button onClick={() => setShowGeneratePayrollModal(false)} className="text-slate-400 hover:text-slate-600">✕</button>
            </div>
            <form onSubmit={handleGeneratePayrollSubmit} className="space-y-3 text-xs">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[10px] font-bold uppercase text-slate-500 mb-1">Special Allowances ({settings.currency})</label>
                  <input
                    type="number"
                    value={payAllowances}
                ...
                <div>
                  <label className="block text-[10px] font-bold uppercase text-slate-500 mb-1">Tax / Deductions ({settings.currency})</label>
                  <input
                    type="number"
                    value={payDeductions}
      ...
      {showNewStaffModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-2xl max-w-md w-full p-6 border border-slate-200">
            <div className="flex justify-between items-center pb-3 border-b border-slate-200 mb-4">
              <h3 className="font-extrabold text-slate-900 text-sm">Add New Staff Employee</h3>
              <button onClick={() => setShowNewStaffModal(false)} className="text-slate-400 hover:text-slate-600">✕</button>
            </div>
            <form onSubmit={handleSaveStaff} className="space-y-3 text-xs">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[10px] font-bold uppercase text-slate-500 mb-1">Monthly Base Salary ({settings.currency})</label>
                  <input
                    type="number"
                    value={empSalary}
